...
SYSTEM_PROMPT = """You are a data extraction assistant. Your job is to read restaurant and bar website text and extract structured happy hour information.

You will receive raw text scraped from a venue's website. Extract every distinct happy hour offering and return them as a JSON array.

Each happy hour entry must have this exact schema:
{
  ...
  "label": "Happy Hour" or "Late Night",    // short descriptor if the venue names their specials
  "confidence": "high" | "medium" | "low"   // how confident you are in the extraction
}

RULES:
1. A venue may have MULTIPLE happy hours (e.g., weekday 4-6pm AND late-night 9-11pm). Return each as a separate entry.
...
5. If the text mentions "happy hour" but gives no timing or day info at all, return an empty array [].
6. Do NOT fabricate times or days. If the text is unclear, return an empty array.
7. Return ONLY valid JSON. No preamble, no markdown code fences, no explanation.
...
def extract_with_llm(text, venue_name=""):
    ...
    if len(text) > MAX_TEXT_CHARS:
        # Focus on regions near "happy hour" mentions
        hh_indices = [m.start() for m in re.finditer(r"happy\s*hour", text, re.IGNORECASE)]
        if hh_indices:
...
VISION_SYSTEM_PROMPT = """You are a data extraction assistant. You will be shown an image that may contain happy hour information from a bar or restaurant (like a drink menu, specials board, or promo graphic).

Extract every distinct happy hour offering visible in the image and return a JSON array.
...
Each entry must have this schema:
{
  ...
  "label": "Happy Hour" or "Late Night",
  "confidence": "high" | "medium" | "low"
}

RULES:
1. Multiple happy hours -> multiple entries.
...
5. If the image is NOT a happy hour/drink menu (e.g., it's a logo, food photo, or interior shot), return an empty array [].
